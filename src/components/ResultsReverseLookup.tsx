...
export function ResultsReverseLookup({ data }: Props) {
  ...
  const results = getNestedValue(data, "results") as Record<string, unknown>[] | undefined;
  const status = getNestedValue(data, "status");
  const message = getNestedValue(data, "message");

  // Error state
  if (status === "error" || (message && !identity && !results)) {
    return (
